...
main()  
# здесь реализуется функция direct_extract, которая позволяетудалять из очереди элемент.
# К двум запросам из  задачи 6.1 добавляется запрос на удаление элемента с индексом i:
# Если элемент с таким индексом не существовет, то в массив который будет в рузельтате добавляем -1, 
#если же был, то добавляем значение этого элемента.